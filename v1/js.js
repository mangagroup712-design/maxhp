document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        triggerTransition(() => {
            ...
            setTimeout(() => blackCurtain.style.display = 'none', 500);
        });
    }, 100);
    ...
    function initGame() {
        ...
        player.crashY = 0; player.crashVy = 0; player.crashRot = 0;
    ...
    function project(p, cameraX, cameraY, cameraZ, fov, horizY) {
        let worldX = p.x - cameraX;
        let worldY = p.y - cameraY;
        let worldZ = p.z - cameraZ;
        if (worldZ <= 0) worldZ = 1;
        let scale = fov / (worldZ / ROAD_WIDTH);
        return { x: centerX + (scale * worldX), y: horizY + (scale * worldY), w: scale * ROAD_WIDTH, scale: scale };
    }
    ...
    function update() {
        ...
        if (player.isCrashed) {
            ...
            player.crashVy += 1.5; 
            player.crashRot += 0.25; 
            
            player.speed *= 0.96;
            player.z += player.speed * 1.6;
            
            // フェードアウト
            if (player.crashY > -100) { 
                blackCurtain.style.display = 'block';
                let fadeProgress = (player.crashY + 100) / 400; 
                blackCurtain.style.opacity = Math.max(0, Math.min(1, fadeProgress));
            }

            // 復帰判定
            if (player.crashY > 300) {
                player.isCrashed = false;
                player.crashY = 0;
                player.crashRot = 0;
                ...
                if (lives <= 0) {
                    endGame();
                    blackCurtain.style.opacity = 0.5;
                } else {
                    invincibility = 2.0; 
                    player.speed = 100;
                    setTimeout(() => { blackCurtain.style.opacity = 0; }, 50);
                    setTimeout(() => { blackCurtain.style.display = 'none'; }, 600);
                }
            }
            updateParticles();
            return;
        ...
        enemies.forEach((enemy, i) => {
            ...
            if (invincibility <= 0 && !player.isCrashed && distZ > -250 && distZ < 650 && distX < 750) {
                ...
                createExplosion(centerX, canvas.height - 150, 2.0); 
            }
        });
    }
    ...
    function drawSportsCar(ctx, x, y, scale, hue, isPlayer, nitroActive) {
        ...
        if (isPlayer && !player.isCrashed) ctx.rotate(currTurn * 0.12);
    ...
    function render() {
        ...
        if (player.isCrashed) {
            // 【修正点1】カメラの高さを少し抑える（0.8倍）
            // これにより、地面が完全に視界から消えるのを防ぎやすくする
            syncY = -player.crashY * 0.8; 
            
            // 【修正点2】画面回転（ロール）を廃止し、わずかな「揺らぎ」に変更
            // 以前は player.crashRot をそのまま使っていたが、
            // sin波を使って「左右に少し揺れる（±2度程度）」だけにする
            syncRot = Math.sin(player.crashRot) * 0.04; 
        }
        ...
        if (uiLayer) {
            // UIも激しく回さず、少し傾く程度にする
            uiLayer.style.transform = `rotate(${syncRot}rad)`;
        }
        ...
        let activeHorizon = horizonY + shakeY + syncY * 0.8;
        ctx.fillRect(-100, activeHorizon, canvas.width+200, canvas.height+200);

        let startPos = Math.floor(player.z / SEGMENT_LENGTH);
        let camX = player.x * 0.7 + currTurn * 1500;
        let camH = currentCamHeight + shakeY + syncY * 2.5;

        for (let n = DRAW_DISTANCE; n > 0; n--) {
            let p1 = project({ x: 0, y: camH, z: (startPos + n) * SEGMENT_LENGTH }, camX, shakeY, player.z, currentFOV, activeHorizon);
            let p2 = project({ x: 0, y: camH, z: (startPos + n - 1) * SEGMENT_LENGTH }, camX, shakeY, player.z, currentFOV, activeHorizon);
            
            if (p1.y <= activeHorizon) continue;
            ...
            if (fog < 0.95) {
                ...
                ctx.beginPath(); ctx.moveTo(p1.x - p1.w, p1.y); ctx.lineTo(p1.x - p1.w - kerbW, p1.y); ctx.lineTo(p2.x - p2.w - kerbW, p2.y); ctx.lineTo(p2.x - p2.w, p2.y); ctx.fill();
                ctx.beginPath(); ctx.moveTo(p1.x + p1.w, p1.y); ctx.lineTo(p1.x + p1.w + kerbW, p1.y); ctx.lineTo(p2.x + p2.w + kerbW, p2.y); ctx.lineTo(p2.x + p2.w, p2.y); ctx.fill();
            }
        ...
        enemies.forEach(e => {
            let p = project({ x: e.x, y: camH, z: e.z }, camX, shakeY, player.z, currentFOV, activeHorizon);
            if (p.scale > 0 && (e.z - player.z) < (DRAW_DISTANCE * SEGMENT_LENGTH) && p.y > activeHorizon) {
        ...
        particles.forEach(p => {
            ...
            ctx.beginPath(); ctx.arc(p.x + shakeX, p.y + shakeY + syncY, p.size, 0, Math.PI * 2); ctx.fill();
        });

        ctx.globalAlpha = (invincibility > 0 && !player.isCrashed) ? (0.4 + Math.sin(Date.now() / 40) * 0.2) : 1.0;
        
        if (gameState !== 'start') {
            let drawY = canvas.height - 50 + shakeY;
            if (player.isCrashed) {
                // カメラは緩やかに追従するが、車体は計算上の位置に正しく描画することで
                // 「車が画面内で暴れている」ように見せる
                // （カメラ追従率0.8なので、車は少し上下に動いて見える＝ダイナミックさを維持）
                drawY += player.crashVy * 0.2; 
            }
            drawSportsCar(ctx, centerX + shakeX, drawY, 0.26, 0, true, keys['Space'] && nitro > 0);
        }
        ctx.restore();
    }
